...
```{r co-occur_plots}
word_pairs %>%
  filter(n >= 100) %>% # first plot had 70
...
# correlation of all words with justice
justice_cors <- word_cors %>% 
  filter(item1 == "justice")

# words correlated with "environmental", "justice", "equity", and "income"
word_cors %>%
...
```{r}
# control for highly common words across all documents
...
tstat_freq <- textstat_frequency(doc_freq_matrix, n = 5, groups = year)
head(tstat_freq, 10)
```
...
```{r convert_dfm}
# two word pairs instead of single words
...
```{r}
# looks at all documents / words, then calculates which words occur more/less frequently than you'd expect in a specific document (statistically)
keyness <- textstat_keyness(doc_freq_matrix_2, # includes all documents
                            target = 1) # refers to first document (2015)
...
```{r hierarch_clust}
dist <- as.dist(textstat_dist(doc_freq_matrix))
clust <- hclust(dist)
