...
skipgrams <- incidents_df %>%
  ...
  # size of window = 5 for checking words related to each other 
  # (isn't a set standard, usually between 5 and 8)
...
normalized_prob <- skipgram_probs %>%
  ...
  left_join(unigram_probs %>%
              select(word1 = word, probability_1 = probability),
            by = "word1") %>%
  left_join(unigram_probs %>%
              select(word2 = word, probability_2 = probability),
            by = "word2") %>%
  mutate(prob_together = probability / probability_1 / probability_2)
# creates matrix with pairs of words
# probability = probability that pair appears together
# probability_1 or 2 = probability of word1 or word2 appearing
# prob_togher = probability that two words show up together


# Which words are most associated with "rope"?   
normalized_prob %>% 
  filter(word1 == "rope") %>%
  arrange(-prob_together)
```
...
word_vectors <- pmi_svd$u # dimensions along which the words co-occur
rownames(word_vectors) <- rownames(pmi_matrix)
```

```{r syn-function}
# create function to take single word vector and compare to all words in all dimension
...
```{r find-synonyms}
# look at similarity scores for "fall" and "slip"
